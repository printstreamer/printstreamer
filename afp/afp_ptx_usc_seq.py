...
class AFP_PTX_USC:
    ...
    def parse(self, data):
        ...
        """
        self.PREFIX, self.CLASS, self.LENGTH, self.TYPE, self.BYPSIDEN = unpack(f">1s1sB1s1s", data)
    ...
    def format(self):
        ...
        :returns: Record data
        """
        data = pack(f">1s1sB1s1s", self.PREFIX, self.CLASS, self.LENGTH, self.TYPE, self.BYPSIDEN)
        return data
