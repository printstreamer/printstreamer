...
afp_ptx_ami_fields_list = [
    ...
    StreamFieldAFP(name="TYPE", offset=3, length=1, type="CODE", optional=True, range_values=["X'C6' -", ''], default=False, indicator=False, meaning=['Control sequence', '']),
    StreamFieldAFP(name="DSPLCMNT", offset=4, length=2, type="SBIN", optional=True, range_values=["X'0000' -", ''], default=False, indicator=False, meaning=['Displacement', '']),
    ]
afp_ptx_ami_fields = {}
for field in afp_ptx_ami_fields_list:
    afp_ptx_ami_fields[field.name] = field
...
class AFP_PTX_AMI:
    ...
    def __init__(self):
        ...
        self.TYPE = None                #      3       1  CODE  X'C6' -       Control sequence                 y    n    n
        self.DSPLCMNT = None            #      4       2  SBIN  X'0000' -     Displacement                     y    n    n
