...
class AFP_PMC:
    ...
    def parse(self, data):
        ...
        """
        pass
        # self.PMCid, self.Reserved_1, self.Triplets = unpack(f">1s1s{len(self.Triplets)}s", data)
    ...
    def format(self):
        ...
        :returns: Record data
        """
        data = pack(f">1s1s{len(self.Triplets)}s", self.PMCid, self.Reserved_1, self.Triplets)
        return data
