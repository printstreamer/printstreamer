...
afp_ptx_sbi_fields_list = [
    ...
    StreamFieldAFP(name="TYPE", offset=3, length=1, type="CODE", optional=True, range_values=["X'D0' -", ''], default=False, indicator=False, meaning=['Control sequence', '']),
    ...
    StreamFieldAFP(name="= 1/6 inch if", offset=240, length=1, type="/1440 inch", optional=True, range_values=['this paramete', ''], default=True, indicator=True, meaning=['r is not specified in an SB', '']),
    ]
afp_ptx_sbi_fields = {}
...
class AFP_PTX_SBI:
    ...
    def __init__(self):
        ...
        self.TYPE = None                #      3       1  CODE  X'D0' -       Control sequence                 y    n    n
        self.INCRMENT = None            #      4       2  SBIN  X'0000' -     Increment                        y    y    y
        self.= 1/6 inch if = None       #    240       1  /144  this paramete r is not specified in an         y    y    y
    ...
    def parse(self, data):
        ...
        """
        self.PREFIX, self.CLASS, self.LENGTH, self.TYPE, self.INCRMENT, self.= 1/6 inch if = unpack(f">1s1sB1sh", data)
    ...
    def format(self):
        ...
        :returns: Record data
        """
        data = pack(f">1s1sB1sh", self.PREFIX, self.CLASS, self.LENGTH, self.TYPE, self.INCRMENT, self.= 1/6 inch if)
        return data
