""" Parse a Xerox Metacode printstream file segment into the generic model. """
...
class StreamSegmentMetacode:
    """ Interpret a Metacode file into one document of model pages. """
    ...
    def parse(self):
        ...
        self.cur_document = Document(
            name=self.name, source_ref=SourceRef(file_format="metacode", record_type="document"))
    ...
    def _on_page(self, width, height, elements):
        ...
        page = Page(number=self.pages, size=Rect(0, 0, width, height),
                    source_ref=SourceRef(file_format="metacode", record_type="page",
