""" Parse a PCL printstream file segment into the generic model. """
...
class StreamSegmentPCL:
    """ Interpret a PCL file into one document of model pages. """
    ...
    def parse(self):
        ...
        self.cur_document = Document(
            name=self.name, source_ref=SourceRef(file_format="pcl", record_type="document"))
        ...
        PCLParser(self._on_page).parse(data)
        if self.options.page_sink:
    ...
    def _on_page(self, width, height, elements):
        ...
        page = Page(number=self.pages, size=Rect(0, 0, width, height),
                    source_ref=SourceRef(file_format="pcl", record_type="page",
